...
public class DefaultListableBeanFactory extends AbstractAutowireCapableBeanFactory
	...
	public DefaultListableBeanFactory() {
		/**
		 * spring的一个工厂
		 * 1.beanDefinitionMap属性，是一个map，key：bean的名字，value：BeanDefinition
		 *  a.BeanDefinition：spring中用于描述bean的对象，类似Java中用于描述类的Class，{@link BeanDefinition}
		 *  b.beanDefinitionNames：存放所有bean的名字
		 *  c.dependencyComparator：用于排序
		 */
		super();
	}
	...
	public String[] getBeanNamesForType(@Nullable Class<?> type, boolean includeNonSingletons, boolean allowEagerInit) {
		/**
		 * 在没有调用 freezeConfiguration()之前这个是一直成立的
		 * 这也是为什么在实例化bean之前会调用freezeConfiguration()
		 */
		if (!isConfigurationFrozen() || type == null || !allowEagerInit) {
			/**
			 * 调用doGetBeanNamesForType()处理
			 */
	...
	private String[] doGetBeanNamesForType(ResolvableType type, boolean includeNonSingletons, boolean allowEagerInit) {
		...
		/**
		 * 先从交给spring的BeanDefinition中查找
		 * 即非调用registerSingleton()添加的
		 */
		for (String beanName : this.beanDefinitionNames) {
			...
			if (!isAlias(beanName)) {
				try {
					 ...
					 * 是从mergedBeanDefinitions获取的，不存在则存到mergedBeanDefinitions（Map）中
					 * 这个属性很重要，bean实例化取的就是这里面的
					 */
		...
		/**
		 * 手动注册的singleton查找
		 * 即调用registerSingleton()注册的
		 * 在实例AnnotatedBeanDefinitionReader时注册了4个
		 * 在调用ConfigurationClassPostProcessor#processConfigBeanDefinitions()添加了1个
		 */
	...
	protected boolean isAutowireCandidate(String beanName, RootBeanDefinition mbd,
		...
		if (mbd.isFactoryMethodUnique && mbd.factoryMethodToIntrospect == null) {// @Bean的时候非静态标记了FactoryMethodUnique
	...
	public void preInstantiateSingletons() throws BeansException {
		...
		for (String beanName : beanNames) {
			/**
			 * 取出BeanDefinition，每次调用getBeanNamesForType()都会把之前没有存到
			 * mergedBeanDefinitions的BeanDefinition转为RootBeanDefinition存到里面
			 */
			RootBeanDefinition bd = getMergedLocalBeanDefinition(beanName);
			/**
			 * 非抽象类，单例，非懒加载
			...
			if (!bd.isAbstract() && bd.isSingleton() && !bd.isLazyInit()) {
				 ...
				 * ->{@link AbstractAutowireCapableBeanFactory#predictBeanType(String, RootBeanDefinition, Class[])}
				 */
				if (isFactoryBean(beanName)) {
	...
	public void registerBeanDefinition(String beanName, BeanDefinition beanDefinition)
		...
		/**
		 * 先从beanDefinitionMap取当前要注册的BeanDefinition
		 * 1.map里存在bd，则进行是否可以覆盖等判断后，再进行注册（判断中不抛异常才会执行）
		 * 2.map里不存在bd，则进行注册
		 */
		...
		/**
		 * 如果map里存在，并且单例池里存在当前bean
		 * 则重置单例池，因为现在还没有到实例化bean的实际
		 */
	...
	public Object doResolveDependency(DependencyDescriptor descriptor, @Nullable String beanName,
		...
		try {
			 ...
			 *
			 * 解析组合属性
			 * spring 寻找依赖的方式 根据Type查找所有的BeanDefinition, 然后返回beanNames(String[])
			 * {@link BeanFactoryUtils#beanNamesForTypeIncludingAncestors(ListableBeanFactory, Class, boolean, boolean)}
			 * 本质还是调用{@link DefaultListableBeanFactory#getBeanNamesForType(java.lang.Class, boolean, boolean)}
			 * 这个方法在spring准备实例化bean之前，如果一个beanName不存在，spring会创建并缓存到里面
			 * 在spring在准备实例化bean的之前调用了{@link #freezeConfiguration()} 所有在此步直接从缓存取
			 *
			 * Map: beanName - Class(非bean)
			 * 如果注入的属性是此种类型, 则spring会将满足的Class全部封装为Map
			 *
			 * Array: 借用Map 存 Value
			 *
			 * Collection: 借用Map 存Value 默认List
			 *
			 * StreamDependencyDescriptor:  TODO
			 *
			 */
			...
			/**
			 *
			 * 普通filed, 无论普通还是组合在进行获取的时候都做了4种依赖Class替换
	...
	protected Map<String, Object> findAutowireCandidates(
		...
		/**
		 * 这里完成了依赖替换 ApplicationContext等4个
		 */
