...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	protected <T> T     doGetBean(final String name, @Nullable final Class<T> requiredType,
		...
		if (sharedInstance != null && args == null) {
			...
			// We're assumably within a circular reference.// 是否正在创建
			/**
			 * 当前创建的bean是不是prototype
			 * 是的话抛出异常
			 */
			...
			try {
				...
				/**
				 * 当前实例的bean是否有依赖的bean，有的话，先实例化依赖的bean
				 */
	...
	protected RootBeanDefinition getMergedLocalBeanDefinition(String beanName) throws BeansException {
		// Quick check on the concurrent map first, with minimal locking. //先从缓存中查找
	...
	protected RootBeanDefinition getMergedBeanDefinition(
		...
		synchronized (this.mergedBeanDefinitions) {
			...
			if (mbd == null) {
				/**
				 * 获取父类的名字，为null
				 * 则将当前BeanDefinition转为RootBeanDefinition
				 */
				if (bd.getParentName() == null) {//转为RootBeanDefinition
	...
	protected Object getObjectForBeanInstance(
		...
		if (BeanFactoryUtils.isFactoryDereference(name)) {
			...
			// beanName以&开头，但又不是factoryBean，直接抛出异常
			if (!(beanInstance instanceof FactoryBean)) {
		...
		// 当前bean是factoryBean，并且beanName不以&开头，才会跳过这个判断执行下面的方法
		// 但是spring默认的factoryBean是以&开头的，而且上面的判断，如果这个beanName以
		// &开头，但是又不是factoryBean会直接抛异常，所以后续代码会执行？ TODO
		if (!(beanInstance instanceof FactoryBean) || BeanFactoryUtils.isFactoryDereference(name)) {
			return beanInstance;
		}
	...
	protected boolean requiresDestruction(Object bean, RootBeanDefinition mbd) {
		/**
		 * 当前bean不是NullBean
		 * 1.当前bean实现了DisposableBean | AutoCloseable | xml配置了destroy方法
		 *
		 * 或者
		 * 2.后置处理器中存在实现DestructionAwareBeanPostProcessor接口
		 *  CommonAnnotationBeanPostProcessor间接实现,并且在第四次后置处理器
		 *  时解析了生命回调方法
		 * 此次就是从里面找回调方法
		 *
		 * TODO 第九次调用后置处理器
		 ...
		 *  查询destroy方法,调用父类的
		 *  {@link org.springframework.beans.factory.annotation.InitDestroyAnnotationBeanPostProcessor#requiresDestruction(Object)}
		 *
		 *
		 */
	...
	protected void registerDisposableBeanIfNecessary(String beanName, Object bean, RootBeanDefinition mbd) {
		 ...
		 * 有destroy方法
		 */
		if (!mbd.isPrototype() && requiresDestruction(bean, mbd)) {// 当前bean不是Prototype
			if (mbd.isSingleton()) {
				...
				/**
				 * 注册destroy方法
				 * 本质就是往disposableBeans(Map)放一个
				 * beanName - bean(封装了执行destroy的后置处理器,当前bean的一个DisposableBean实例)
				 */
