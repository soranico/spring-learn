...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	...
	protected Object createBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		/**
		 * spring中吧look-up method和replace method标记为override
		 * 这个在spring官网有说明
		 ...
		 * https://docs.spring.io/spring/docs/5.2.6.RELEASE/spring-framework-reference/core.html#beans-factory-arbitrary-method-replacement
		 * TODO 测试
		 */
		try {
		...
		try {
			 ...
			 * 在bean初始化之前
			 * 判断用户是否有自己的定义bean的方式
			 * i.e 用户自己继承InstantiationAwareBeanPostProcessor或者其子类
			 * 在其中的postProcessBeforeInstantiation()返回一个实例
			 * 则spring会把它放入容器，后续的属性填充等操作不会再执行，但会初始化回调方法会执行
			 */
	...
	protected Object doCreateBean(final String beanName, final RootBeanDefinition mbd, final @Nullable Object[] args)
		...
		/**
		 * singleton的话，则从存储factoryBean的map中移除（如果存在）
		 * 一般不是factoryBean是不可能存在这里面的
		 */
		if (mbd.isSingleton()) {
		...
		/**
		 * 创建实例，注意不是一个完整的bean
		 * 一个完整的bean应该是属性都填充完成的bean
		 * 这一步只是创建了bean并没有填充属性，所以不是一个完成bean
		 */
		...
		synchronized (mbd.postProcessingLock) {
			if (!mbd.postProcessed) {
				try {
					/**
					 *
					 * 执行后置处理器修改合并BeanDefinition
					 * 完成解析注册回调方法，解析@Resource的CommonAnnotationBeanPostProcessor
					 * 完成解析注册@Autowired的AutowiredAnnotationBeanPostProcessor
					 * 完成向singletonNames里put(beanName,isSingleton)的ApplicationListenerDetector
					 *
					 */
		...
		if (!Boolean.FALSE.equals(mbd.beforeInstantiationResolved)) {
			...
			if (!mbd.isSynthetic() && hasInstantiationAwareBeanPostProcessors()) {
				...
				if (targetType != null) {
					 ...
					 * 3.ImportAwareBeanPostProcessor 继承SmartInstantiationAwareBeanPostProcessor
					 * 4.AnnotationAwareAspectJAutoProxyCreator继承SmartInstantiationAwareBeanPostProcessor(如果开启代理支持的话会有)
					 * 这一次调用如果开启了代理支持会过滤掉不需要代理的类
					 * 如果没有开启代理支持，这一步什么都不会做
					 */
	...
	protected BeanWrapper createBeanInstance(String beanName, RootBeanDefinition mbd, @Nullable Object[] args) {
		...
		/**
		 *
		 * 有可用构造器 | 注入模型为构造器注入  | 手动为BeanDefinition添加了值 | 参数不为空
		 * 则用构造方法完成注入
		 * 以后找时间测一下
		 * TODO 测试
		 * mbd.setConstructorArgumentValues()设置值，工厂后置处理器搞一下
		 */
	...
	protected Constructor<?>[] determineConstructorsFromBeanPostProcessors(@Nullable Class<?> beanClass, String beanName)
		...
		if (beanClass != null && hasInstantiationAwareBeanPostProcessors()) {
			 ...
			 *
			 * AnnotationAwareAspectJAutoProxyCreator间接实现（需要开启动态代理才会有） 什么都没有做
			 * 大部分情况下返回的都是null，除非存在注解构造器或者有参构造器
			 * ImportAwareBeanPostProcessor 间接实现 什么都没有做
			 *
			 */
			for (BeanPostProcessor bp : getBeanPostProcessors()) {
				if (bp instanceof SmartInstantiationAwareBeanPostProcessor) {
	...
	protected BeanWrapper instantiateBean(final String beanName, final RootBeanDefinition mbd) {
		try {
			...
			if (System.getSecurityManager() != null) {
				...
				/**
				 * getInstantiationStrategy()获取bean的生成策略 调用SimpleInstantiationStrategy
				 * 默认为CglibSubclassingInstantiationStrategy
				 */
