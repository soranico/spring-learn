...
public class AspectJAwareAdvisorAutoProxyCreator extends AbstractAdvisorAutoProxyCreator {
	...
	protected boolean shouldSkip(Class<?> beanClass, String beanName) {
		...
		/**
		 * 调用{@link AnnotationAwareAspectJAutoProxyCreator#findCandidateAdvisors()}
		 *
		 * 首先解析spring自己的切面，e.g 开启事务的话会有事务的解析 beanName存到cachedAdvisorBeanNames
		 * 解析spring自己的调用{@link AbstractAdvisorAutoProxyCreator#findCandidateAdvisors()}
		 *
		 * 其次解析用户的@Aspect
		 * 调用{@link BeanFactoryAspectJAdvisorsBuilder#buildAspectJAdvisors()}
		 */
